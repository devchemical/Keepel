...
const SheetContent = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Content>, SheetContentProps>(
  ({ side = "right", className, children, ...props }, ref) => {
    // Para bottom sheet, usar padding adaptativo
    const isBottom = side === "bottom"
    const paddingClass = isBottom ? "p-4 pb-safe" : "p-6"

    return (
      <SheetPortal>
        <SheetOverlay />
        <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), paddingClass, className)} {...props}>
          {/* Drag handle para bottom sheets - patrón moderno */}
          {isBottom && <div className="bg-muted-foreground/20 mx-auto mb-4 h-1.5 w-12 rounded-full" />}
          {children}
